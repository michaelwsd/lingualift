...
import { WordMatchingExercise } from '@/types';
...
export const WordMatching: React.FC<WordMatchingProps> = ({ exercise, onComplete }) => {
  ...
  const allMatched = matches.length === exercise.words.length;
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          {exercise.definitions.map(({ id, text }) => {
      ...
      {checked && (
        <div className="text-center text-sm text-stone-500 mt-4">
          {Object.values(results).filter(v => v).length} of {exercise.words.length} correct
        </div>
      )}
    </div>
  );
